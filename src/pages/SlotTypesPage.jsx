...
import { addSlotType, updateSlotType, deleteSlotType } from "../store/slotTypesSlice";
...
const SlotTypesPage = () => {
  ...
  const handleAdd = () => {
    if (newType.trim()) {
      dispatch(addSlotType({ name: newType.trim() }));
      setNewType('');
      toast.success('Slot type added');
    }
  };
  ...
  const handleUpdate = () => {
    if (editName.trim()) {
      dispatch(updateSlotType({ id: editing, name: editName.trim() }));
      setEditing(null);
      toast.success('Slot type updated');
    }
  };

  const handleDelete = (id) => {
    const type = slotTypes.find(t => t.id === id);
    const inUse = slots.some(s => s.type === type.name);
    if (inUse) {
      toast.error('Cannot delete slot type in use');
      return;
    }
    dispatch(deleteSlotType(id));
    toast.success('Slot type deleted');
  };
